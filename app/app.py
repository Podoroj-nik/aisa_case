...
def call_gigachat_simple(prompt: str, source: str) -> dict:
    try:
        ...
        text = (
            f"[ДЕМО-РЕЖИМ] Библиотека gigachat не установлена.\n\n"
            f"Промпт: {prompt[:200]}...\n\n"
            f"Установите: pip install gigachat"
        )
        tokens_used = 0
    ...
    print(f"[{ts}] source={source} | tokens={tokens_used} | total_afisha={token_stats['afisha']} | total_dialog={token_stats['dialog']}")
...
class GigaChatClient:
    ...
    async def chat(self, messages: list[dict]) -> str:
        try:
            ...
            return "[ДЕМО-РЕЖИМ] Библиотека gigachat не установлена. Установите: pip install gigachat"
        except Exception as e:
            raise HTTPException(status_code=502, detail=f"Ошибка GigaChat: {str(e)}")
...
async def api_afisha_ai_details(request: Request, event: EventAIRequest):
    ...
    print(f"[LOG] guest afisha | eventId={event.eventId} | tokens={result['tokens_consumed']}")
...
async def list_chats(request: Request):
    ...
    async with session_scope(factory) as session:
        ...
        return [
            ChatOut(id=c.id, title=c.title, created_at=c.created_at)
            for c in chats
        ]
